...
"""
TARGET直線ロジック採点アプリ
５．１６直線２.csv 実データ列順対応版 + 推奨馬SVG + 買い目表示 + 重賞モード v11_1
...
def create_picks_svg(rec_df: pd.DataFrame) -> str:
    # SVG版の推奨馬画像。内部ランク文言は出さず、買い目だけを整理して表示。
    bg = "#020B1D"
    gold = "#E3BF4D"
    white = "#F6F6F6"
    gray = "#8D96A6"
    ...
    for _, row in rec_df.iterrows():
        ...
        # 買い目のみ表示
        parts.append(f'<text x="340" y="{y + 146}" fill="{soft}" font-family="{font_family}" font-size="28" font-weight="600">ワイド：{wide}</text>')
        parts.append(f'<text x="340" y="{y + 184}" fill="{soft}" font-family="{font_family}" font-size="28" font-weight="600">馬連：{exacta}｜3複：{trio}</text>')
        ...
        if comment and trifecta not in ["", "見送り"]:
            third_line += f"｜{comment}"
        elif comment and trifecta in ["", "見送り"]:
            third_line += f"｜{comment}"
...
st.caption("v12.2互換CSV + 通常推奨SVG（買い目表示付き）+ 重賞ランキングSVG v11_1")
