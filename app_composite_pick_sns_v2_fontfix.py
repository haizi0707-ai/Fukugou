...
def draw_sns_image(df, image_date=""):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        no = norm_text(row.get("馬番", ""))
        bb = draw.textbbox((0, 0), no, font=f_num)
        nw = bb[2] - bb[0]
        nh = bb[3] - bb[1]
        draw.text((cx - nw / 2, cy - nh / 2 - 4), no, font=f_num, fill=bg)

        # Horse name
        x_name = 370
        horse_name = str(row.get("馬名", "")).strip()
        horse_name = fit_text(draw, horse_name, f_name, max_width=770)
        draw.text((x_name, y_top + 18), horse_name, font=f_name, fill=white)
        ...
        if sub:
            draw.text((x_name, y_top + 90), sub, font=f_sub, fill=gray)
...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合ランキングと参考画像に近いSNS投稿用画像を1枚で出力します。")
