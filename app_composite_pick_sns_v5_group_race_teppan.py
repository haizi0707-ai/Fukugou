...
def build_image_rows(df):
    ...
    for (date, place, race), g in base.groupby(["日付", "競馬場", "R"], sort=False):
        ...
        if len(teppan_only) > 0:
            ...
            segments = []
            names = []
            for j, (_, tr) in enumerate(teppan_only.iterrows()):
                nm = str(tr.get("馬名", "")).strip()
                if not nm:
                    continue
                names.append(nm)
                if j > 0:
                    segments.append(("、", "#F5F2EA"))
                segments.append((nm, color_by_teppan_rank(tr.get("鉄板ランク", ""))))
...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合候補を全頭表示し、参考画像に近いSNS投稿用画像を1枚で出力します。超鉄板⭐️は赤、鉄板⭐️/強鉄板⭐️は黄で表示し、同レースの鉄板候補は1行にまとめます。")
