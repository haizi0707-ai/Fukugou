...
def build_teppan_group_sub_segments(group_df, include_top=False):
    """
    鉄板単独レースのサブ表示用。
    基本は本命馬を除いた相手だけを「○ 1 ▲ 10」のように並べる。
    """
    g = group_df.copy()
    if g.empty:
        return []
...
def build_image_rows_from_sources(straight_src_df, teppan_src_df, final_df):
    ...
    ・鉄板のみレースは、そのレースの1番手を主役表示し、下に印＋馬番を表示
    ・直線ロジック馬と鉄板が被る場合は、馬名色だけ変えて重複行は出さない
    ・並び順は競馬場ごとにレース昇順
    """
    straight_norm = normalize_straight_df(straight_src_df)
    ...
    if not teppan_norm.empty:
        ...
        teppan_norm = (
            ...
            .drop_duplicates(subset=["競馬場", "R", "馬番", "馬名"], keep="first")
            .copy()
        )

        for (_, place, race), g in teppan_norm.groupby(["日付", "競馬場", "R"], dropna=False, sort=False):
            g = g.copy()
            ...
            rr["表示相手"] = ""
            rr["表示サブセグメント"] = build_teppan_group_sub_segments(g, include_top=False)
            rr["表示順"] = 1
...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合候補を全頭表示し、参考画像に近いSNS投稿用画像を1枚で出力します。超鉄板⭐️は赤、鉄板⭐️/強鉄板⭐️は黄で表示し、鉄板CSVの超鉄板/鉄板/強鉄板は全頭拾い、鉄板のみレースは1番手の馬名を大きく表示し、その下に印＋馬番を直線ロジック風に表示します。直線ロジックと被る場合は文字色だけ変更し、ランキングは変えません。")
