...
def build_image_rows_from_sources(straight_src_df, teppan_src_df, final_df):
    """
    画像専用の最終表示行を作る。
    ・直線ロジック馬は個別表示（相手表示は直線ロジック側そのまま）
    ・鉄板のみレースは、そのレースの鉄板馬を1行に集約して
      1行目「2 ルートサーティーン」
      2行目「3 ダークマルス  8 ナムラヒナギク」
      のように表示する
    ・直線ロジックと被る馬は馬名色だけ変更し、重複行は出さない
    ・並び順は競馬場ごとにレース昇順
    """
    ...
    def simple_key(place, race, no):
        return f"{norm_text(place)}|{to_int_safe(race)}|{norm_text(no)}"

    # 1) 直線ロジック側（ランキングは変更しない）
    ...
    for _, r in straight_rows.iterrows():
        ...
        rr["表示順"] = 0
        rows.append(rr)
        shown_keys.add(simple_key(rr.get("競馬場", ""), rr.get("R", ""), rr.get("馬番", "")))
    ...
    if not teppan_norm.empty:
        ...
        # 直線ロジックで既に表示済みの馬を除いたうえで、レース単位で集約
        keep_mask = []
        for _, tr in teppan_norm.iterrows():
            keep_mask.append(simple_key(tr.get("競馬場", ""), tr.get("R_num", ""), tr.get("馬番", "")) not in shown_keys)
        teppan_only = teppan_norm[keep_mask].copy()

        if not teppan_only.empty:
            teppan_only = teppan_only.sort_values(
                ["venue_order", "R_num", "_mark_priority", "_rank_priority", "馬番_num"],
                ascending=[True, True, True, True, True]
            )

            for (_, place, rnum), g in teppan_only.groupby(["日付", "競馬場", "R_num"], sort=False, dropna=False):
                ...
                top = g.iloc[0].copy()
                rr = top.copy()
                rr["R"] = f"{int(rnum)}R" if pd.notna(rnum) else top.get("R", "")
                ...
                rr["表示順"] = 1
                rr["straight_present"] = False
                rr["teppan_present"] = True
                rr["複合ラベル"] = composite_label(False, rr.get("鉄板ランク", ""))
...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合候補を全頭表示し、参考画像に近いSNS投稿用画像を1枚で出力します。超鉄板⭐️は赤、鉄板⭐️/強鉄板⭐️は黄で表示し、鉄板CSVの超鉄板/鉄板/強鉄板は全頭拾い、鉄板のみレースは左の丸に「激アツ」と表示し、1行目に「2 ルートサーティーン」、2行目に「3 ダークマルス  8 ナムラヒナギク」のようにまとめて表示します。直線ロジックと被る場合は文字色だけ変更し、ランキングは変えません。")
