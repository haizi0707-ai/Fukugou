...
"""
TARGET直線ロジック採点アプリ
５．１６直線２.csv 実データ列順対応版 + 推奨馬SVG + 買い目表示 + 重賞モード v11_0
...
def build_buy_advice_columns(rec_df: pd.DataFrame) -> pd.DataFrame:
    """
    推奨ランク別に、ワイド・馬連・三連複・三連単の買い目表示を整理する。
    ワイドは回収率未検証なのでROIは表示しない。
    """
    out = rec_df.copy()
...
def build_major_race_ranking(rank_df: pd.DataFrame, only_major_name: bool = True, manual_targets_text: str = "") -> pd.DataFrame:
    ...
    for race_key, g in rank_df.groupby("race_key"):
        ...
        if "芝ダ替わり" in danger or "前4角85%超" in danger:
            ...
            major_bet = "見送り寄り / ワイドのみ少額なら可"
        elif gap >= 12:
            reliability = "S"
            comment = "抜けた本命候補。"
            major_bet = "ワイド◎-○▲△ / 馬連◎-○▲△ / 3複◎-2〜6位 / 3単◎軸M2〜5位"
        elif gap >= 8:
            reliability = "A"
            comment = "本命候補。"
            major_bet = "ワイド◎-○▲△ / 馬連◎-○▲△ / 3複◎-2〜6位 / 3単は軽め"
        elif gap >= 4:
            reliability = "B"
            comment = "軸候補だが相手広め。"
            major_bet = "ワイド◎-○▲△ / 馬連◎-○▲△ / 3複◎-2〜6位"
        else:
            reliability = "C"
            comment = "混戦。単独本命は危険。"
            major_bet = "見る重賞 / 買うならワイド少額"
...
def create_major_svg(major_df: pd.DataFrame) -> str:
    ...
    if len(major_df) == 0:
        ...
        for _, g in major_df.groupby("race_key"):
            ...
            if major_bet:
                parts.append(f'<text x="{margin_x}" y="{y + 130}" fill="{accent}" font-family="{font_family}" font-size="28" font-weight="700">買い目：{svg_escape_text(major_bet)}</text>')
...
def create_picks_svg(rec_df: pd.DataFrame) -> str:
    ...
    for _, row in rec_df.iterrows():
        ...
        rec_rank = svg_escape_text(row.get("推奨ランク", ""))
        buy_rank = svg_escape_text(row.get("買い方ランク", ""))
        wide = svg_escape_text(row.get("ワイド買い目", ""))
        ...
        rank_color = red if rec_rank in ["超強推奨", "強推奨"] or buy_rank in ["激アツ", "勝負", "攻め候補"] else gold
        ...
        # ランク
        parts.append(f'<text x="340" y="{y + 140}" fill="{rank_color}" font-family="{font_family}" font-size="30" font-weight="800">[{rec_rank}] 買い方:{buy_rank}</text>')

        # 買い目
        parts.append(f'<text x="340" y="{y + 176}" fill="{soft}" font-family="{font_family}" font-size="27" font-weight="600">ワイド：{wide}</text>')
        parts.append(f'<text x="340" y="{y + 210}" fill="{soft}" font-family="{font_family}" font-size="27" font-weight="600">馬連：{exacta}｜3複：{trio}</text>')
        ...
        if comment:
            third_line += f"｜{comment}"

        parts.append(f'<text x="340" y="{y + 244}" fill="{gold}" font-family="{font_family}" font-size="26" font-weight="700">{svg_escape_text(third_line)}</text>')
...
st.caption("v12.2互換CSV + 通常推奨SVG（買い目表示付き）+ 重賞ランキングSVG v11_0")
