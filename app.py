...
"""
TARGET直線ロジック採点アプリ
５．１６直線２.csv 実データ列順対応版 + 推奨馬SVG + 買い目表示 + 重賞モード v11_3
...
INTERNAL_EXTRA_COLUMNS = ["間隔", "休み明け戦目", "前走着差", "前走上3F順位"]
...
def calc_tenkai_break_score(row: pd.Series) -> int:
    """
    前走だけで作る展開打破点。
    順位は変えず、信頼度タグとして使う。
    """
    ...
    agari = to_int(row.get("前走上3F順位", ""))
    prev_dist = to_int(row.get("前走距離数値", ""))
    cur_dist = to_int(row.get("距離", ""))
    interval = to_int(row.get("間隔", ""))
    layoff = to_int(row.get("休み明け戦目", ""))

    score = 0

    # 前で運べる
    if prev_field and p4 and (p4 / prev_field) <= 0.45:
        score += 25
    ...
    # 上がりを使える
    if agari is not None and agari <= 5:
        score += 20

    # 後方からでも脚を使えて着差小
    if (
        prev_field and p4 and (p4 / prev_field) >= 0.45
        and agari is not None and agari <= 3
        and margin is not None and margin <= 0.5
    ):
        score += 15
    ...
    # ローテが極端に悪くない
    if interval is not None and interval < 8 and layoff is not None and layoff >= 2:
        score += 5

    return int(score)


def judge_tenkai_break(score: Any) -> str:
    n = to_int(score, None)
    if n is None:
        return ""
    if n >= 85:
...
def judge_safety(danger_count: Any) -> str:
    n = to_int(danger_count, 0)
    if n <= 0:
        return "安全◎"
    if n == 1:
        return "注意"
...
def build_total_tag(safety: str, tenkai: str) -> str:
    tags = []
    if safety:
        tags.append(safety)
    if tenkai:
...
def date_code_from_text(s: str) -> str:
    """
    画像右上用の日付表記。
    2026.5.17 / 517 / 0517 などを 5/17 に変換する。
    """
    t = norm(s)
...
def convert_target_to_v12(df: pd.DataFrame, year: int, filter_7_12: bool, min_field_if_blank: int):
    ...
    for _, row in df.iterrows():
        ...
        temp_rows.append({
            ...
            "前走着差": margin if margin is not None else "",
            "前走上3F順位": agari_rank if agari_rank is not None else "",
            "_finish": finish,
            "_margin": margin,
            "_going": going,
...
def build_ranking_df(v12_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        danger_counts.append(len(dangers))

        tenkai_score = calc_tenkai_break_score(r)
        tenkai_judge = judge_tenkai_break(tenkai_score)
        safety_judge = judge_safety(len(dangers))

        tenkai_scores.append(tenkai_score)
        tenkai_judges.append(tenkai_judge)
        safety_judges.append(safety_judge)
        total_tags.append(build_total_tag(safety_judge, tenkai_judge))

    df["追加補正点"] = correction_points
    df["補正理由"] = correction_reasons
    df["危険理由"] = danger_reasons
    df["危険理由数"] = danger_counts
    df["安全判定"] = safety_judges
    df["展開打破点"] = tenkai_scores
    df["展開打破判定"] = tenkai_judges
    df["総合タグ"] = total_tags
...
def build_buy_advice_columns(rec_df: pd.DataFrame) -> pd.DataFrame:
    """
    推奨ランク別に、単勝・複勝・ワイド・馬連・三連複・三連単を整理する。
    画像は「推奨買い方」をシンプル表示し、詳細はCSV/内部に保持する。
    """
    out = rec_df.copy()
    ...
    for _, r in out.iterrows():
        ...
        win_bets.append(win)
        place_bets.append(place)
        wide_bets.append(wide)
    ...
    out["推奨買い方"] = bet_summaries
    out["単勝買い目"] = win_bets
    out["複勝買い目"] = place_bets
    out["ワイド買い目"] = wide_bets
...
def is_major_race_name(name: Any) -> bool:
    """
    重賞判定。
    以前は「賞・杯・ステークス」も拾っていたため、特別戦まで重賞扱いになっていました。
    ここでは G1/G2/G3/Jpn 表記がある場合だけ自動重賞扱いにします。
    """
    s = norm(name).upper()
    patterns = [
        "G1", "G2", "G3",
...
def parse_manual_major_targets(text: str) -> List[str]:
    """
    手入力の重賞対象を分解。
    例：
    東京11R
...
def is_manual_major_match(place: Any, race: Any, targets: List[str]) -> bool:
    if not targets:
        return False
    ...
    if r_num is not None:
        candidates.add(f"{p}{r_num}R")
        candidates.add(f"{p}{r_num}")
        candidates.add(f"{r_num}R")
        candidates.add(str(r_num))
    candidates.add(f"{p}{r}")

    target_set = {norm(t).replace(" ", "") for t in targets}
    cand_set = {norm(c).replace(" ", "") for c in candidates}
    return bool(target_set & cand_set)


def build_major_race_ranking(rank_df: pd.DataFrame, only_major_name: bool = True, manual_targets_text: str = "") -> pd.DataFrame:
    rows = []
    manual_targets = parse_manual_major_targets(manual_targets_text)
    ...
    for race_key, g in rank_df.groupby("race_key"):
        ...
        place = g.iloc[0].get("場所", "")
        race = g.iloc[0].get("レース", "")

        if manual_targets:
            if not is_manual_major_match(place, race, manual_targets):
                continue
        elif only_major_name and not is_major_race_name(race_name):
            continue
        ...
        if "芝ダ替わり" in danger or "前4角85%超" in danger:
            ...
            reliability = "S"
            comment = "抜けた本命候補。"
            major_bet = "単勝、複勝、ワイド、馬連、三連複、三連単"
        elif gap >= 8:
            reliability = "A"
            comment = "本命候補。"
            major_bet = "単勝、複勝、ワイド、馬連、三連複"
        elif gap >= 4:
            reliability = "B"
            comment = "軸候補だが相手広め。"
            major_bet = "単勝、複勝、ワイド、馬連、三連複"
        ...
        for _, r in g.head(6).iterrows():
            rows.append({
                ...
                "重賞買い目": major_bet if int(r["順位"]) == 1 else "",
                "補正理由": r.get("補正理由", ""),
                "危険理由": r.get("危険理由", ""),
                "危険理由数": r.get("危険理由数", ""),
                "安全判定": r.get("安全判定", ""),
...
def create_major_svg(major_df: pd.DataFrame) -> str:
    ...
    if len(major_df) == 0:
        ...
        for _, g in groups:
            height += race_header_h + row_h * len(g) + gap_h
    ...
    if len(major_df) == 0:
        ...
        for _, g in major_df.groupby("race_key"):
            ...
            rel = first["重賞信頼度"]
            gap = first["2位との差"]
            comment = str(first.get("短評", ""))
            major_bet = str(first.get("重賞買い目", ""))
            ...
            if comment:
                parts.append(f'<text x="{margin_x}" y="{y + 92}" fill="{gray}" font-family="{font_family}" font-size="28" font-weight="500">{svg_escape_text(comment)}</text>')
            if major_bet:
                parts.append(f'<text x="{margin_x}" y="{y + 130}" fill="{accent}" font-family="{font_family}" font-size="28" font-weight="700">推奨買い方：{svg_escape_text(major_bet)}</text>')
...
def create_picks_svg(rec_df: pd.DataFrame) -> str:
    # SVG版の推奨馬画像。買い方は「推奨買い方」のみシンプル表示。
    bg = "#020B1D"
    gold = "#E3BF4D"
    white = "#F6F6F6"
    gray = "#8D96A6"
    ...
    if date_code:
        parts.append(f'<text x="{width - margin_x}" y="155" fill="{gold}" font-family="{font_family}" font-size="44" font-weight="500" text-anchor="end" letter-spacing="4">{svg_escape_text(date_code)}</text>')
    ...
    for _, row in rec_df.iterrows():
        ...
        bet_summary = svg_escape_text(row.get("推奨買い方", ""))
        total_tag = svg_escape_text(row.get("総合タグ", ""))
        ...
        parts.append(f'<text x="340" y="{y + 58}" fill="{white}" font-family="{font_family}" font-size="54" font-weight="800">{horse_name}</text>')
        parts.append(f'<text x="340" y="{y + 102}" fill="{gray}" font-family="{font_family}" font-size="29" font-weight="500">{partner}</text>')
        parts.append(f'<text x="340" y="{y + 150}" fill="{gold}" font-family="{font_family}" font-size="30" font-weight="700">推奨買い方：{bet_summary}</text>')
        if total_tag:
            parts.append(f'<text x="340" y="{y + 190}" fill="{soft}" font-family="{font_family}" font-size="28" font-weight="700">タグ：{total_tag}</text>')
...
st.caption("v12.2互換CSV + 通常推奨SVG（買い目表示付き）+ 重賞ランキングSVG v11_3")
...
with st.expander("今回の抽出基準", expanded=True):
    ...
    - 自動判定はG1/G2/G3/Jpn表記があるレースのみ
    - G表記が無い場合は「東京11R」のように手入力で指定
    - 信頼度S/A/B/C/Dで表示
...
major_only_name = st.checkbox("重賞ランキングはG1/G2/G3/Jpn表記のあるレースだけ自動表示", value=True)
manual_major_text = st.text_input(
    "重賞対象を手入力する場合（例：東京11R, 京都11R）",
    value="",
    help="レース名にG表記が無い場合はここで指定してください。入力がある場合はこの指定を優先します。"
)
...
if uploaded is not None:
    ...
    rank_view_cols = [
        ...
        "順位", "印", "補正理由", "危険理由", "危険理由数", "安全判定", "展開打破点", "展開打破判定", "総合タグ"
    ]
    ...
    if output_mode in ["両方", "通常推奨"]:
        ...
        if len(rec_df) == 0:
            ...
            view_cols = [
                ...
                "追加補正点", "補正理由", "危険理由",
                "危険理由数", "安全判定", "展開打破点", "展開打破判定", "総合タグ",
                "買い方ランク", "推奨買い方", "単勝買い目", "複勝買い目", "ワイド買い目", "馬連買い目", "三連複買い目", "三連単買い目", "買い目点数", "検証ROI", "買い方コメント",
                "○", "▲", "△", "他1", "他2"
            ]
            ...
            st.subheader("⑦ 通常推奨SVG（iPhoneはスクショ保存推奨）")
    ...
    if output_mode in ["両方", "重賞ランキング"]:
        ...
        major_df = build_major_race_ranking(rank_df, only_major_name=major_only_name, manual_targets_text=manual_major_text)

        if len(major_df) == 0:
            st.warning("重賞候補が見つかりませんでした。レース名にG表記が無い場合は、上の手入力欄に例：東京11R のように指定してください。")
        else:
