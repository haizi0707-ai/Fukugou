...
def build_image_rows_from_sources(straight_src_df, teppan_src_df, final_df):
    ...
    画像専用の最終表示行を作る。
    ・直線ロジック馬は個別表示
    ・鉄板CSVの超鉄板/鉄板/強鉄板は全頭拾う
    ・直線ロジック馬と被った鉄板馬は直線側の馬名色だけ変更
    ・同じレースに鉄板のみ候補が複数いる場合は1行にまとめる
    """
    ...
    if not straight_rows.empty:
        ...
        straight_rows = straight_rows.sort_values(["日付", "競馬場", "R_num", "複合点", "馬番_num"], ascending=[True, True, True, False, True])
    ...
    # 2. 鉄板CSVから全頭を拾い、直線ロジックと被っていない馬をレースごとにまとめる
    if not teppan_norm.empty:
        for c in ["日付", "競馬場", "R", "馬番", "馬名", "鉄板ランク", "判定"]:
        ...
        teppan_norm = (
            teppan_norm.sort_values(["競馬場", "R_num", "馬番_num", "_rank_priority"], ascending=[True, True, True, False])
                       .drop_duplicates(subset=["競馬場", "R", "馬番", "馬名"], keep="first")
        )

        for (date, place, race), g in teppan_norm.groupby(["日付", "競馬場", "R"], dropna=False, sort=False):
            g = g.copy()

            # 直線ロジック側で表示済みの馬はここでは出さない
            keep = []
            for _, tr in g.iterrows():
                keep.append(simple_key(tr.get("競馬場", ""), tr.get("R", ""), tr.get("馬番", "")) not in shown_keys)
            g = g[keep]

            if g.empty:
                continue

            g = g.sort_values(["馬番_num"], ascending=[True])
            segments, display_text = build_group_display_segments(g)
            ...
            rr["表示馬番"] = "鉄"
            rr["表示相手"] = ""
            rr["鉄板ランク"] = strongest_teppan_rank(g["鉄板ランク"].tolist())
            rr["straight_present"] = False
            rr["teppan_present"] = True
            rr["複合ラベル"] = composite_label(False, rr["鉄板ランク"])
            rr["複合点"] = 80 + teppan_rank_priority(rr["鉄板ランク"]) * 10
...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合候補を全頭表示し、参考画像に近いSNS投稿用画像を1枚で出力します。超鉄板⭐️は赤、鉄板⭐️/強鉄板⭐️は黄で表示し、鉄板CSVの超鉄板/鉄板/強鉄板は全頭拾い、同レースの鉄板のみ候補は1行にまとめ、長い場合は「馬番+馬名3文字」や馬番のみの短縮表示にします。")
