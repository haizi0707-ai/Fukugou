...
def find_font(patterns: List[str], fallback_paths: List[str]) -> Optional[str]:
    # fc-match 優先
    for p in patterns:
        file_path = run_cmd(["fc-match", "-f", "%{file}", p])
...
def get_font_paths() -> Tuple[Optional[str], Optional[str]]:
    regular = find_font(
        ["Noto Sans CJK JP", "Noto Sans JP", "Noto Sans CJK", "Sans"],
        [
    ...
    bold = find_font(
        ["Noto Sans CJK JP Bold", "Noto Sans JP Bold", "Noto Sans CJK Bold", "Sans Bold"],
        [
...
def create_picks_image(rec_df: pd.DataFrame) -> Image.Image:
    ...
    font_small_en = load_font(bold_path, 30)
    font_title = load_font(bold_path, 74)
    font_date = load_font(regular_path, 44)
    font_track = load_font(bold_path, 46)
    font_race = load_font(bold_path, 44)
    font_no = load_font(bold_path, 42)
    font_sub = load_font(regular_path, 32)
    ...
    for _, row in rec_df.iterrows():
        ...
        name_font = fit_font(draw, str(row["馬名"]), bold_path, 58, 42, max_name_w)
...
st.caption("５．１６直線２.csv 実データ列順対応版 / v12.2互換CSV + 本日の推奨馬PNG v4")
