...
APP_VERSION = "vCOURSE_RACEINFO_PCI_FIX_2026_05_20"
...
<text x="610" y="126" font-size="15" font-weight="700" fill="#475569">点</text>
''']
    medals = {1: "🥇", 2: "🥈", 3: "🥉"}
    y0 = 140
    for i, (_, r) in enumerate(d.iterrows()):
        ...
        score = "" if pd.isna(r.get("ペーススコア", np.nan)) else f"{float(r.get('ペーススコア')):.1f}"
        ...
        svg.append(f'<text x="230" y="{y+27}" font-size="17" font-weight="700" fill="#111827">{name}</text>')
...
st.caption("前走PCIの 55.1* などを正しく読み込めるように修正済み。pandas.read_html/lxmlは使っていません。")
...
show_cols = ["日付", "場所", "R", "レース名", "開催日目", "開催週区分", "コース区分", "馬番", "馬名", "騎手", "調教師", "評価", "判定理由", "ペース順位", "ペーススコア", "今回ペース型", "基準使用区分", "条件件数", "騎手ペース点", "調教師ペース点", "前走RPCI", "前走PCI", "前走PCI_raw", "前走Ave3F", "前走上3F地点差", "前4角_使用", "前走頭数"]
...
st.caption("注：詳細条件の母数が30件未満の場合は、自動で粗い条件へフォールバックします。PCI列は 55.1* のようなTARGET表記も数値化します。")
