...
"""
TARGET直線ロジック採点アプリ
５．１６直線２.csv 実データ列順対応版 + 推奨馬SVG + 買い目表示 + 重賞モード v10_9
...
def build_buy_advice_columns(rec_df: pd.DataFrame) -> pd.DataFrame:
    """
    推奨ランク別に、馬連・三連複・三連単の買い目表示を整理する。
    ここは採点ではなく、最終出力用の買い方表示。
    """
    out = rec_df.copy()
...
def build_major_race_ranking(rank_df: pd.DataFrame, only_major_name: bool = True, manual_targets_text: str = "") -> pd.DataFrame:
    ...
    for race_key, g in rank_df.groupby("race_key"):
        ...
        if "芝ダ替わり" in danger or "前4角85%超" in danger:
            ...
            reliability = "S"
            comment = "抜けた本命候補。"
            major_bet = "3単◎軸M2〜5位 / 馬連◎-○▲△ / 3複◎-2〜6位"
        elif gap >= 8:
            reliability = "A"
            comment = "本命候補。"
            major_bet = "馬連◎-○▲△ / 3複◎-2〜6位 / 3単は軽め"
        elif gap >= 4:
            reliability = "B"
            comment = "軸候補だが相手広め。"
            major_bet = "馬連◎-○▲△ / 3複◎-2〜6位"
        else:
            reliability = "C"
            comment = "混戦。単独本命は危険。"
            major_bet = "見る重賞・買うなら少額"
...
def create_picks_svg(rec_df: pd.DataFrame) -> str:
    ...
    for _, row in rec_df.iterrows():
        ...
        # 馬名
        parts.append(f'<text x="340" y="{y + 62}" fill="{white}" font-family="{font_family}" font-size="54" font-weight="800">{horse_name}</text>')
        parts.append(f'<text x="340" y="{y + 106}" fill="{gray}" font-family="{font_family}" font-size="29" font-weight="500">{partner}</text>')

        # ランク
        parts.append(f'<text x="340" y="{y + 146}" fill="{rank_color}" font-family="{font_family}" font-size="30" font-weight="800">[{rec_rank}] 買い方:{buy_rank}</text>')

        # 買い目
        parts.append(f'<text x="340" y="{y + 184}" fill="{soft}" font-family="{font_family}" font-size="28" font-weight="600">馬連：{exacta}</text>')
        parts.append(f'<text x="340" y="{y + 218}" fill="{soft}" font-family="{font_family}" font-size="28" font-weight="600">3複：{trio}</text>')
        ...
        parts.append(f'<text x="340" y="{y + 250}" fill="{gold}" font-family="{font_family}" font-size="26" font-weight="700">{svg_escape_text(third_line)}</text>')
...
st.caption("v12.2互換CSV + 通常推奨SVG（買い目表示付き）+ 重賞ランキングSVG v10_9")
