...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合候補を全頭表示し、参考画像に近いSNS投稿用画像を1枚で出力します。超鉄板⭐️は赤、鉄板⭐️/強鉄板⭐️は黄で表示し、同レースの鉄板候補は1行にまとめ、長い場合は「馬番+馬名3文字」や馬番のみの短縮表示にします。")
...
if run:
    try:
        ...
        image_df = build_image_rows(final_df)
