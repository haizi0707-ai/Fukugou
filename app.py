...
def dedupe_composite_rows(df):
    ...
    sort_cols = ["日付", "競馬場", "R_num", "馬番_num", "複合点"]
    work = work.sort_values(sort_cols, ascending=[True, True, True, True, False]).copy()
...
def short_display_name(name, n=3):
    s = str(name).strip()
    return s[:n] if len(s) > n else s


def build_group_display_segments(teppan_only):
    """
    同レースに鉄板候補が複数いる時の表示用。
    まず「馬番+馬名3文字」で並べ、長すぎる場合は馬番だけへ短縮する。
    """
    compact_items = []
    number_items = []

    for _, tr in teppan_only.iterrows():
        no = norm_text(tr.get("馬番", ""))
        nm = short_display_name(tr.get("馬名", ""), n=3)
        color = color_by_teppan_rank(tr.get("鉄板ランク", ""))
        if no and nm:
            compact_items.append((f"{no}{nm}", color))
        elif nm:
            compact_items.append((nm, color))
        elif no:
            compact_items.append((no, color))

        if no:
            number_items.append((no, color))

    def join_segments(items):
        segs = []
        texts = []
        for i, (txt, col) in enumerate(items):
            ...
            if i > 0:
                segs.append(("、", "#F5F2EA"))
            segs.append((txt, col))
        return segs, "、".join(texts)

    segs1, text1 = join_segments(compact_items)
    # だいたい16〜18文字を超えると1行に収まりづらいので馬番だけへ縮約
    if len(text1) <= 18:
        return segs1, text1

    segs2, text2 = join_segments(number_items)
    return segs2, text2


def build_image_rows(df):
    """
    画像表示用に、同レースの鉄板のみ候補を1行にまとめる。
    直線ロジック馬は個別表示し、同じレースの鉄板のみ候補は
    「馬名、馬名、馬名」のように1行へ集約する。
    """
    if df.empty:
        return df.copy()

    base = dedupe_composite_rows(df).copy()
    rows = []

    for (date, place, race), g in base.groupby(["日付", "競馬場", "R"], sort=False):
        g = g.copy()
        g["馬番_num"] = g["馬番"].map(to_int_safe).fillna(999)

        straight_rows = g[g["straight_present"] == True].sort_values(["複合点", "馬番_num"], ascending=[False, True])
        teppan_only = g[(g["straight_present"] != True) & (g["teppan_present"] == True)].sort_values(
            ["鉄板ランク"], key=lambda s: s.map(teppan_rank_priority), ascending=False
        ).sort_values(["馬番_num"], ascending=[True], kind="stable")

        # 直線ロジック馬はそのまま表示
        for _, r in straight_rows.iterrows():
            rr = r.copy()
            rr["表示モード"] = "single"
            rr["表示名"] = rr.get("馬名", "")
            rr["表示名セグメント"] = [(str(rr.get("馬名", "")).strip(), color_by_teppan_rank(rr.get("鉄板ランク", "")))]
            rr["表示馬番"] = norm_text(rr.get("馬番", ""))
            rr["表示相手"] = norm_text(rr.get("相手表示", ""))
            rows.append(rr)

        # 同レースの鉄板のみ候補は1行へまとめる
        if len(teppan_only) > 0:
            rr = teppan_only.iloc[0].copy()
            rr["表示モード"] = "group"

            segments, display_text = build_group_display_segments(teppan_only)

            rr["表示名"] = display_text
            rr["表示名セグメント"] = segments
            rr["表示馬番"] = "鉄"
            rr["表示相手"] = ""
            rr["鉄板ランク"] = strongest_teppan_rank(teppan_only["鉄板ランク"].tolist())
            rows.append(rr)
...
def mark_priority(mark):
    m = norm_text(mark)
    order = {"◎": 1, "本命": 1, "○": 2, "対抗": 2, "▲": 3, "単穴": 3, "△": 4, "連下": 4, "☆": 5, "注": 6, "他": 7}
    return order.get(m, 99)


def venue_priority(place):
    order = {
        "札幌": 1, "函館": 2, "福島": 3, "新潟": 4, "東京": 5,
        "中山": 6, "中京": 7, "京都": 8, "阪神": 9, "小倉": 10
    }
    return order.get(norm_text(place), 99)



def build_teppan_group_name_segments(group_df):
    """
    鉄板のみレースの1行目表示。
    例: 2 ルートサーティーン
    """
    ...
    if g.empty:
        return []
    top = g.iloc[0]
    no = norm_text(top.get("馬番", ""))
    name = str(top.get("馬名", "")).strip()
    color = color_by_teppan_rank(top.get("鉄板ランク", ""))
    txt = f"{no} {name}".strip() if no else name
    return [(txt, color)] if txt else []


def build_teppan_group_sub_segments(group_df):
    """
    鉄板のみレースの2行目表示。
    例: 3 ダークマルス  8 ナムラヒナギク
    先頭馬は除外し、2番手以降だけを並べる。
    """
    g = group_df.copy()
    if g.empty:
    ...
    for i, (_, r) in enumerate(g.iterrows()):
        if i == 0:
            continue
        no = norm_text(r.get("馬番", ""))
        name = str(r.get("馬名", "")).strip()
        color = color_by_teppan_rank(r.get("鉄板ランク", ""))
        txt = f"{no} {name}".strip() if no else name
        if txt:
            items.append((txt, color))
...
def build_image_rows_from_sources(straight_src_df, teppan_src_df, final_df):
    """
    画像専用の最終表示行を作る。

    方針
    ・同じレースに鉄板馬が複数いる場合は、そのレースを1つの欄に集約する
      例:
      京都5R  [激アツ]
      2 ルートサーティーン
      3 ダークマルス  8 ナムラヒナギク
    ・直線ロジック馬と鉄板が被る場合も、そのレースが集約対象なら個別行は出さず集約欄にまとめる
    ・直線ロジックのみ / 鉄板1頭のみ(かつ直線ロジックあり) は従来通り個別表示
    ・並び順は競馬場ごとにレース昇順
    """
    teppan_norm = normalize_teppan_df(teppan_src_df)
    final_base = dedupe_composite_rows(final_df).copy()

    rows = []

    # ---- 直線ロジック存在レースの把握 ----
    straight_race_keys = set()
    if not final_base.empty:
        tmp_st = final_base[final_base["straight_present"] == True].copy()
        if not tmp_st.empty:
            tmp_st["R_num"] = tmp_st["R"].map(to_int_safe).fillna(99)
            for _, r in tmp_st.iterrows():
                straight_race_keys.add(
                    (norm_text(r.get("日付", "")), norm_text(r.get("競馬場", "")), int(r.get("R_num", 99)))
                )

    # ---- 鉄板データ整形 ----
    grouped_race_keys = set()
    teppan_group_rows = []

    if not teppan_norm.empty:
        for c in ["日付", "競馬場", "R", "馬番", "馬名", "鉄板ランク", "判定", "印"]:
            if c not in teppan_norm.columns:
                teppan_norm[c] = ""

        teppan_norm["venue_order"] = teppan_norm["競馬場"].map(venue_priority)
        teppan_norm["R_num"] = teppan_norm["R"].map(to_int_safe).fillna(99)
        teppan_norm["馬番_num"] = teppan_norm["馬番"].map(to_int_safe).fillna(999)
        teppan_norm["_rank_priority"] = teppan_norm["鉄板ランク"].map(teppan_rank_priority).fillna(0)
        teppan_norm["_mark_priority"] = teppan_norm["印"].map(mark_priority).fillna(99)

        # 同馬重複除去
        teppan_norm = (
            teppan_norm.sort_values(
                ["venue_order", "R_num", "_mark_priority", "_rank_priority", "馬番_num"],
                ascending=[True, True, True, False, True]
            )
            .drop_duplicates(subset=["日付", "競馬場", "R_num", "馬番", "馬名"], keep="first")
            .copy()
        )

        # 集約対象レースを決める
        # 1) 同レースに鉄板馬が2頭以上いるレース
        # 2) 鉄板1頭だけでも、直線ロジックが存在しないレースは集約表示にする
        for (date, place, rnum), g in teppan_norm.groupby(["日付", "競馬場", "R_num"], sort=False, dropna=False):
            g = g.copy().sort_values(
                ["_mark_priority", "_rank_priority", "馬番_num"],
                ascending=[True, False, True]
            ).reset_index(drop=True)

            race_key = (norm_text(date), norm_text(place), int(rnum) if pd.notna(rnum) else 99)

            if len(g) >= 2 or race_key not in straight_race_keys:
                grouped_race_keys.add(race_key)

                top = g.iloc[0].copy()
                rr = top.copy()
                rr["R"] = top.get("R", f"{int(rnum)}R" if pd.notna(rnum) else "")
                rr["表示モード"] = "teppan_group"
                rr["表示馬番"] = "激アツ"
                rr["表示名"] = norm_text(top.get("馬名", ""))
                rr["表示名セグメント"] = build_teppan_group_name_segments(g)
                rr["表示相手"] = ""
                rr["表示サブセグメント"] = build_teppan_group_sub_segments(g)
                rr["表示順"] = 0
                rr["straight_present"] = bool(race_key in straight_race_keys)
                rr["teppan_present"] = True
                rr["複合ラベル"] = composite_label(rr["straight_present"], rr.get("鉄板ランク", ""))
                rr["複合点"] = 80 + teppan_rank_priority(rr.get("鉄板ランク", "")) * 10
                rr["総合ランク"] = composite_rank_label(rr["複合点"])
                teppan_group_rows.append(rr)

    # ---- 直線ロジック個別行 ----
    if not final_base.empty:
        straight_rows = final_base[final_base["straight_present"] == True].copy()
        if not straight_rows.empty:
            straight_rows["venue_order"] = straight_rows["競馬場"].map(venue_priority)
            straight_rows["R_num"] = straight_rows["R"].map(to_int_safe).fillna(99)
            straight_rows["馬番_num"] = straight_rows["馬番"].map(to_int_safe).fillna(999)
            straight_rows = straight_rows.sort_values(
                ["venue_order", "R_num", "複合点", "馬番_num"],
            ...
            for _, r in straight_rows.iterrows():
                ...
                # 集約対象レースは個別行を出さない
                if race_key in grouped_race_keys:
                    continue
    ...
    # ---- 集約済み鉄板行を追加 ----
    rows.extend(teppan_group_rows)

    if not rows:
        return pd.DataFrame()

    out = pd.DataFrame(rows)
    out["venue_order"] = out["競馬場"].map(venue_priority)
    out["R_num"] = out["R"].map(to_int_safe).fillna(99)
    out["馬番_num"] = out["馬番"].map(to_int_safe).fillna(999)
    if "表示順" not in out.columns:
        out["表示順"] = 0

    out = out.sort_values(
        ["venue_order", "R_num", "表示順", "複合点", "馬番_num"],
        ascending=[True, True, True, False, True],
    ).reset_index(drop=True)
    return out


def draw_colored_segments(draw, x, y, segments, font, default_fill, max_width=None):
    cur_x = x
    if segments is None:
        return
    ...
    if max_width is not None:
        ...
        for text, fill in segments:
            ...
            w = bb[2] - bb[0]
            if x + total + w <= x + max_width:
                trimmed.append((text, fill))
                total += w
            else:
                remain = x + max_width - (x + total)
                if remain > 20:
                    s = text
                    while s:
                        bb2 = draw.textbbox((0, 0), s + "…", font=font)
                        if bb2[2] - bb2[0] <= remain:
                            trimmed.append((s + "…", fill))
                            break
                        s = s[:-1]
                break
        segments = trimmed
    ...
    for text, fill in segments:
        if not text:
            continue
        draw.text((cur_x, y), text, font=font, fill=fill or default_fill)
        bb = draw.textbbox((0, 0), text, font=font)
        cur_x += bb[2] - bb[0]
...
def draw_sns_image(df, image_date=""):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        display_mode = norm_text(row.get("表示モード", "single"))
        ...
        if display_mode == "teppan_group":
            f_hot = load_font(22, bold=True)
            try:
                draw.text((cx, cy), "激アツ", font=f_hot, fill=bg, anchor="mm")
            except Exception:
                bb = draw.textbbox((0, 0), "激アツ", font=f_hot)
                nw = bb[2] - bb[0]
                nh = bb[3] - bb[1]
                draw.text((cx - nw / 2, cy - nh / 2), "激アツ", font=f_hot, fill=bg)
        else:
            horse_no = norm_text(row.get("表示馬番", row.get("馬番", "")))
            try:
        ...
        # horse name / grouped teppan names
        name_segments = row.get("表示名セグメント", None)
        if display_mode == "teppan_group":
            font_group = load_font(42, bold=True)
            draw_colored_segments(draw, x_name, y_top + 24, name_segments, font_group, default_fill=white, max_width=760)

            sub_segments = row.get("表示サブセグメント", None)
            if sub_segments:
                draw_colored_segments(draw, x_name, y_top + 82, sub_segments, font_group, default_fill=white, max_width=760)
        elif display_mode == "group":
            font_name = load_font(44, bold=True)
            draw_colored_segments(draw, x_name, y_top + 28, name_segments, font_name, default_fill=white, max_width=760)

            sub_segments = row.get("表示サブセグメント", None)
            if sub_segments:
                draw_colored_segments(draw, x_name, y_top + 90, sub_segments, f_sub, default_fill=gray, max_width=760)
            else:
                opp = norm_text(row.get("表示相手", row.get("相手表示", "")))
            ...
            horse_name = str(row.get("表示名", row.get("馬名", ""))).strip()
...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合候補を全頭表示し、参考画像に近いSNS投稿用画像を1枚で出力します。超鉄板⭐️は赤、鉄板⭐️/強鉄板⭐️は黄で表示し、鉄板CSVの超鉄板/鉄板/強鉄板は全頭拾い、同じレースに鉄板馬が複数いる場合は1つの欄に集約します。左の丸に「激アツ」と表示し、1行目に「2 ルートサーティーン」、2行目に「3 ダークマルス  8 ナムラヒナギク」のようにまとめて表示します。直線ロジックと被る場合も同じ欄に集約し、個別行は出しません。")
...
if run:
    try:
        ...
        image_df = build_image_rows_from_sources(straight_df, teppan_df, final_df)

        st.success(f"複合該当 {len(final_df)}件を作成しました。画像表示 {len(image_df)}行。")
        ...
        # Image
        img = draw_sns_image(image_df, image_date=image_date)
