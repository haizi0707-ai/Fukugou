...
"""
TARGET直線ロジック採点アプリ
５．１６直線２.csv 実データ列順対応版 + 推奨馬SVG + 買い目表示 + 重賞モード v10_1
...
st.caption("v12.2互換CSV + 通常推奨SVG（買い目表示付き）+ 重賞ランキングSVG v10_1")
...
if uploaded is not None:
    ...
    if df.shape[1] < len(TARGET_COLUMNS_37):
        st.warning(f"想定列数 {len(TARGET_COLUMNS_37)}列に対して、このCSVは {df.shape[1]}列です。不足列は空欄扱いになります。")
    elif df.shape[1] > len(TARGET_COLUMNS_37):
        st.info(f"想定列数 {len(TARGET_COLUMNS_37)}列より多い列があります。余分な列は未使用列として読み込みます。")
