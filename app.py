...
def build_image_rows_from_sources(straight_src_df, teppan_src_df, final_df):
    ...
    ・鉄板のみレースは、そのレースの1番手を主役表示し、下に相手の鉄板馬を
      「③ダークマルス  ⑧ナムラヒナギク」のように表示
    ・直線ロジック馬と鉄板が被る場合は、馬名色だけ変えて重複行は出さない
    ・並び順は競馬場ごとにレース昇順
    """
    ...
    def simple_key(place, race, no):
        return f"{norm_text(place)}|{norm_text(race)}|{norm_text(no)}"
    ...
    if not teppan_norm.empty:
        ...
        # 直線ロジックと被る馬を除いたうえで、レース単位にまとめる
        keep_mask = []
        for _, tr in teppan_norm.iterrows():
            keep_mask.append(simple_key(tr.get("競馬場", ""), tr.get("R", ""), tr.get("馬番", "")) not in shown_keys)
        teppan_only = teppan_norm[keep_mask].copy()

        if not teppan_only.empty:
            grouped = teppan_only.groupby(["競馬場", "R_num"], sort=False)

            for (place, rnum), g in grouped:
                ...
                rr["表示名セグメント"] = [(norm_text(top.get("馬名", "")), color_by_teppan_rank(top.get("鉄板ランク", "")))]
                rr["表示馬番"] = norm_text(top.get("馬番", ""))
                rr["表示相手"] = ""
...
def draw_sns_image(df, image_date=""):
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        # subtitle: straight logic opponents / teppan marks
...
st.caption("直線ロジックCSVと鉄板⭐️血統CSVを読み込み、複合候補を全頭表示し、参考画像に近いSNS投稿用画像を1枚で出力します。超鉄板⭐️は赤、鉄板⭐️/強鉄板⭐️は黄で表示し、鉄板CSVの超鉄板/鉄板/強鉄板は全頭拾い、鉄板のみレースは1番手の馬名を大きく表示し、その下に「③ダークマルス  ⑧ナムラヒナギク」のように相手馬を表示します。直線ロジックと被る場合は文字色だけ変更し、ランキングは変えません。")
