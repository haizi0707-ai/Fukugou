...
def draw_ticket_poster(cards, image_date=""):
    ...
    header_h = 180
    ticket_gap = 22
    ticket_w = width - margin_x * 2

    f_title = load_font(72, bold=True)
    f_sub_en = load_font(34, bold=True)
    f_date = load_font(34, bold=False)
    f_legend = load_font(22, bold=True)
    f_ticket_no = load_font(52, bold=True)
    f_header = load_font(34, bold=True)
    f_header_small = load_font(18, bold=False)
    f_mark = load_font(28, bold=True)
    f_num = load_font(30, bold=True)
    f_name = load_font(28, bold=True)
    f_other = load_font(28, bold=True)
    f_flag = load_font(28, bold=True)
    f_footer = load_font(16, bold=True)

    # Calculate dynamic height
    ticket_heights = []
    for card in cards:
        line_count = max(1, len(card.get("lines", [])))
        body_h = 22 + line_count * 54 + 10
        footer_h = 54
        ticket_h = 92 + body_h + footer_h
        ticket_heights.append(ticket_h)
    height = top + header_h + sum(ticket_heights) + ticket_gap * max(0, len(cards) - 1) + 30
    ...
    sub_y = top + 92
    draw.line((72, sub_y + 18, 210, sub_y + 18), fill=gold, width=2)
    draw.line((430, sub_y + 18, 570, sub_y + 18), fill=gold, width=2)
    draw.text((224, sub_y), subtitle, font=f_sub_en, fill=gold)
    ...
    draw.text((legend_x, legend_y + 28), "熱", font=f_legend, fill=gold)
    draw.text((legend_x + 28, legend_y + 28), "=鉄板", font=f_legend, fill=white)
    draw.text((legend_x, legend_y + 56), "印", font=f_legend, fill=white)
    draw.text((legend_x + 28, legend_y + 56), "=直線ロジック", font=f_legend, fill=white)
    ...
    for idx, card in enumerate(cards, start=1):
        ...
        # header strip
        draw.rounded_rectangle((x0 + 8, y0 + 8, x1 - 8, y0 + 88), radius=14, fill=navy, outline=gold2, width=2)
        ...
        box_x1 = box_x0 + 94
        box_y1 = box_y0 + 58
        draw.rounded_rectangle((box_x0, box_y0, box_x1, box_y1), radius=10, outline=gold2, width=2, fill=navy)
        num_txt = str(idx)
        bb = draw.textbbox((0, 0), num_txt, font=f_ticket_no)
        draw.text((box_x0 + (box_x1 - box_x0 - (bb[2] - bb[0])) / 2, box_y0 + (box_y1 - box_y0 - (bb[3] - bb[1])) / 2 - 2), num_txt, font=f_ticket_no, fill=gold)

        # venue/R
        head_txt = f"{norm_text(card.get('競馬場', ''))} {norm_text(card.get('R', ''))}"
        hb = draw.textbbox((0, 0), head_txt, font=f_header)
        draw.text((x0 + (ticket_w - (hb[2] - hb[0])) / 2, y0 + 20), head_txt, font=f_header, fill=white)
        race_name = norm_text(card.get("レース名", ""))
        if race_name:
            race_name = fit_text_to_width(draw, race_name, f_header_small, 400)
            rb = draw.textbbox((0, 0), race_name, font=f_header_small)
            draw.text((x0 + (ticket_w - (rb[2] - rb[0])) / 2, y0 + 58), race_name, font=f_header_small, fill=white)
        ...
        num_w = 72
        flag_w = 44
        name_x = content_x + mark_w + num_w + 8
        flag_x = x1 - 74

        for i, line_row in enumerate(lines):
            yy = line_y + i * 54
            draw.line((x0 + 22, yy + 42, x1 - 22, yy + 42), fill=line, width=1)
            ...
            if mark:
                draw.text((content_x, yy), mark, font=f_mark, fill=ink)
            if num:
                draw.text((content_x + mark_w, yy), num, font=f_num, fill=ink)

            if mark == "他":
                txt = fit_text_to_width(draw, name, f_other, x1 - (content_x + mark_w + 20) - 24)
                draw.text((content_x + mark_w, yy), txt, font=f_other, fill=ink)
            else:
                max_name_w = flag_x - name_x - 18
                txt = fit_text_to_width(draw, name, f_name, max_name_w)
                draw.text((name_x, yy), txt, font=f_name, fill=ink)
                if flag:
                    draw.text((flag_x, yy), flag, font=f_flag, fill=flag_color)

        # footer
        footer_y = y1 - footer_h + 8
        draw.text((x0 + 18, footer_y + 8), "TODAY’S PICKS", font=f_footer, fill=navy)
        draw.line((x0 + 195, footer_y, x0 + 195, footer_y + 32), fill=line, width=2)
        bc_x = x0 + 230
        bc_y = footer_y + 2
        draw_fake_barcode(draw, bc_x, bc_y, 240, 24, color=ink)
        code = f"{norm_text(card.get('競馬場', ''))[:1]}{norm_text(card.get('R', '')).replace('R','').zfill(2)}-{date_text.replace('.','') or '00000000'}-{str(idx).zfill(3)}"
        draw.text((bc_x + 10, bc_y + 26), code, font=f_footer, fill=ink)
